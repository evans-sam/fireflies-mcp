			...
			if (format === "bullet_points") {
				...
				if (transcript.summary.topics_discussed) {
					summary +=
						...
						safeJoin(transcript.summary.topics_discussed, "; ") +
						". ";
				}
				...
				if (transcript.summary.action_items) {
					summary +=
						...
						safeJoin(transcript.summary.action_items, "; ") +
						". ";
				}
...
export async function startHttpServer(
	...
	const fireflies = new FirefliesServer(apiKey);
	...
	const handleMcpRequest = async (req: Request): Promise<Response> => {
		...
		await fireflies.mcpServer.connect(transport);
	...
	return {
		...
		close: async () => {
			...
			await fireflies.stop();
		},
	};
}

// Main execution — only run when this file is the entry point
if (import.meta.main) {
	...
	if (transport === "http") {
		...
		startHttpServer(apiKey, port).catch((error) => {
			process.stderr.write(
